#!/usr/bin/env python3
"""Check a command investigation report (report.md) for the JAGMAN chain and parts.

Usage:  python3 investigation_check.py report.md
...
PARTS = ["Preliminary statement", "Findings of fact", "Opinions", "Recommendations", "Enclosures"]
...
def main():
    ...
    if 1 not in encls or not re.search(r"convening order", encls.get(1, ""), re.I):
        fails.append("enclosure (1) must be the convening order")
    ...
    co = re.search(r"Convening order:\s*([0-9]{1,2} \w+ [0-9]{4})", text)
    due = re.search(r"Due:\s*([0-9]{1,2} \w+ [0-9]{4})", text)
    if co and due:
        d1, d2 = parse_date(co.group(1)), parse_date(due.group(1))
        if d1 and d2 and (d2 - d1).days > 30 and not re.search(r"extension", section(text, "Preliminary statement") or "", re.I):
            fails.append(f"due date is {(d2 - d1).days} days after the convening order and no extension is noted in the preliminary statement")
    else:
        warns.append("convening order date and due date lines not both found in the header")
    if not re.search(r"Classification:", text):
        warns.append("no classification line in the header")
    ps = section(text, "Preliminary statement") or ""
    for need, pat in (("evidence collected or forthcoming", r"reasonably available evidence"), ("custody of original evidence", r"original evidence|custod")):
        if not re.search(pat, ps, re.I):
            warns.append(f"preliminary statement does not address {need}")
    ...
    print(f"INVESTIGATION CHECK: {sys.argv[1]}  {len(ff)} findings, {len(ops)} opinions, {len(recs)} recommendations, {len(encls)} enclosures")
    for w in warns:
        print(f"  WARN  {w}")
    for f in fails:
        print(f"  FAIL  {f}")
