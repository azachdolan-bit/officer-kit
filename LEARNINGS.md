...
Candidate lessons for the plugin under `plugins/officer-kit/`, captured by the `aar` or `debrief` skill (or by hand) and waiting for review by `inspect`. Nothing here changes a skill until a person approves it and the skill's evals pass.

Format, one entry per lesson:

```
## YYYY-MM-DD  <target skill>  <high | medium | low>  -> <plugin | overrides | exemplar>
...
## 2026-09-22  significance-reviewer  high  -> plugin
...
## 2026-09-22  capture-source  high  -> plugin
...
## 2026-09-22  qc-gates  medium  -> plugin
...
## 2026-09-05  qc-gates  high  -> plugin
...
## 2026-09-05  think  medium  -> plugin
...
Status: approved 0.12.0 (build-a-skill checklist)

## 2026-09-04  naval-letter  high  -> plugin
...
Status: approved 0.12.0 (already in the tool: measure_pdf.py Gate 2b, identity read from the rules file)

## 2026-09-04  discrepancy-report  high  -> plugin
...
Status: approved 0.12.0 (gate order already in qc-gates; the priority order is a unit rule, in the author's rules file)
...
Status: approved 0.12.0 (build-a-skill checklist and kit standard 5)
...
Status: approved 0.12.0 (already in precision_check.py: three per kind and a count)
