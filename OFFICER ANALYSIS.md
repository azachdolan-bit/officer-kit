...
Mechanical checks catch what judgment misses. The WHO surgical checklist cut complications from 11 to 7 percent and deaths from 1.5 to 0.8 percent across 7,688 patients; Boorman's design rules are five to nine items, defined pause points, the items experts forget, one page, tested in real conditions. Anthropic's skill guidance says the same operationally: run the validator, fix, repeat; scripts solve rather than defer. The kit's checkers exist. The intended next step was wiring them as hooks that block a save until they pass. **Tested 5 September 2026 and the answer is no, not in Cowork:** a session's project root is the cloud container, not the user's connected folder, so a hook configured in that folder is never read. The checkers therefore stay steps the skills run and gates the reviewers enforce, and they can be blocking hooks only for Claude Code run locally with the working folder as its root. Where the checker cannot block, the reviewer is what makes it hard to skip.
