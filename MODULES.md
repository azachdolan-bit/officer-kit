...
| `endorsement` | "endorse this request", "first endorsement on" | The identification line, From, To, Via, the basic letter's own Subj, and the action in the first sentence with a reason the addressee can check; renders and measures on the letter grid | the endorsements MCO 1900.16 and NAVMC 4000.5D reproduce; SECNAV M-5216.5 is not in the library | Available (later ordinals, same page endorsements, and continued lettering unverified) |
| `memo` | "memo for the record", "point paper", "decision paper" | The five staff papers the publication prints, each in its appendix's parts and words; the MFR renders on the letter grid | MCTP 3-30A ch 3 and App A to E; MCO 5216.20B ch 13 | Available (the From and To memorandum and the business letter need SECNAV M-5216.5, not in the library) |
