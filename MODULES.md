...
| `quiz-builder` | "make a Kahoot", "quiz deck", "host key" | Kahoot decks from the user's copy of the official template, answers placed without a readable pattern, every file read back, host answer and page key | the user's captured source and Kahoot template | Available (0.1.0) |
...
5. **After:** `order-analysis`, `tactical-planning`, `call-for-fire` (quiz-builder shipped in 0.14.0; the walkthrough is a `study-guide` format); a workshop cut verified content free by script.
